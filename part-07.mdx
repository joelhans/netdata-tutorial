...
This part of the guide aims to get you more familiar with features of the dashboard 
...
The settings tab at the top of your Netdata dashboard houses browser settings. 
These settings have no effect on the operation of your Netdata server / daemon. 
...
This tab is a collection of options focused on the performance of the
 web browser by tweaking the default dashboard behaviour. 

You can set when to refresh the charts, which dimensions to show and how you to handle hidden charts, and page scroll.

### Synchronization

This tab is a collection of options focused on the how your metrics and charts synchronize. 

You can choose **parallel** or **sequential** chart refresh policy to use. 
Selecting **parallel** allows you options as to how you prefer re-sync chart refreshes.

You can also choose to sync or not sync hover selection on all chart, pan and zoom on all charts.
...
You can always check if there is an update available right from the Update tab of your Netdata dashboard.
...
Netdata automatically updates itself but if you choose to update it manually 
there are [several well documented methods]() to achieve that, 
however it is recommended that you first go over the [changelog](https://docs.netdata.cloud/changelog/).
...
The snapshot will be downloaded as a file, to your computer, that can be imported back into any netdata dashboard, 
you do not need to import it back to the current server.

Snapshots are not uploaded to a server rather they are handled by your web browser on your computer.

They are temporary because on the reload of your browser, the snapshot is being replaced by the actual 
and live Netdata dashboard.
...
### To export or save a snapshot
 ...
 You save the snapshot file after it has been generated on your browser and like a regular file attachment, 
 you can email the it to your colleague.
